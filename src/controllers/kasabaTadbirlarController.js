...
exports.create = async (req, res) => {
  try {
    ...
    if (missingFields.length > 0) {
      return res
        .status(400)
        .json({ message: `Quyidagi ${missingFields} to'ldirilmagan!` });
    }
    ...
    const tadbir = new kasabaTadbirlarModel({
      ...
      category: { uz: category_uz, ru: category_ru, oz: category_oz },
      users,
      mediaType,
    });
    ...
    res
      .status(201)
      .json({ message: "Ma'lumot muvaffaqiyatli yaratildi!", tadbir });
  } catch (e) {
...
exports.update = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Tadbir muvaffaqiyatli yangilandi", tadbir });
  } catch (e) {
    res.status(500).json({ message: "Serverda xatolik", error: e.message });
  }
};
